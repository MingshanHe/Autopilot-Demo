...
def astardetect(image_filename):
	...
	sp = image.shape
	list_images = [[blank_image for i in range(sp[0])] for i in range(sp[1])]  # array of list of images

	#maze = [[0 for i in range(17)] for i in range(13)]  # matrix to represent the grids of individual cropped images
	maze = np.zeros((sp[0],sp[1]), np.uint8)
...
def callback(data):
	...
	current_point, next_point, nopath_flag = astardetect(cv_img)
	cv2.waitKey(3)

if __name__ =="__main__":
	# # change filename to check for other images
	# image_filename = "/home/hemingshan/auto_ws/src/obstacle/test_images"
	...
	rospy.Subscriber('/camera_rgb_optical_frame/color/image_raw',Image, callback)
	rospy.spin()
